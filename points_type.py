...
class Points_type():
    """
    Class for determine point types located in the grid. 'NN' method uses this
    class to determine which type of numerical scheme should be used.

    Atributes:
    ---------
    grid: torch.Tensor or np.array
            points that require a type definition
    --------
    """
    ...
    def shift_points(grid, axis, shift):
        """
        Shifts all values of an array 'grid' on a value 'shift' in a direcion
        of axis 'axis', somewhat is equivalent to a np.roll

        Atributes:
        ---------
        grid: array (torch, numpy..)
        ---------
        axis: int
        ---------
        shift: float
    ...
    def in_hull(p, hull):
        ...
        of the coordinates of `M` points in `K`dimensions for which Delaunay
    ...
    def point_typization(self):
        """
        Parameters
        ----------
        self.grid : torch.Tensor (torch.float64)
        ...
        point_type : dict
            ...
            is a dimension n
            'f' means that if we add small number to a position of
            corresponding coordinate we stay in the 'hull'
    ...
    def grid_sort(self):
        ...
        self.grid
        ----------
        point_type : reult of point_typization (dict)
            dictionary point:type with a points in a 'grid' above
            type may be 'central' - inner point
            and string of 'f' and 'b', where the length of the string
            is a dimension n
        ...
        -------
        grid_dict : dict
            dictionary type:points list
            basically reversed dictionary
        """
    ...
    def bnd_sort(self, grid_dict, b_coord):
        """
        Method for 'b_coord' type ('central', 'fb',..) definition by
        comparison with 'grid_dict' which cosists of all grid points.

        Parameters:
        ----------
        grid_dict: dict
            dictionary (keys is points type, values is corresponding points).
        ----------
        b_coord: torch.tensor or [torch.tensor,torch.tensor,..]
            boundary points of grid. It will be list if periodic condition is
        ----------
        return: dict or [bnd_dict,bnd_dict]
