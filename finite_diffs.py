...
class First_order_scheme():
    ...
    are used for boundary points. 'cantral', and combination 'f','b' are
    corresponding to points_type.

    Parameters:
    ----------
    term: list
        differentiation direction, example: [0,0]->d2u/dx2 if x is first
        direction in the grid.
    ----------
    nvars: int
        task parameters, if grid(x,t) -> nvars = 2
    ----------
    axes_scheme_type: string
        scheme type: 'cantral' or combination of 'f' and 'b'

    """

    def __init__(self, term, nvars, axes_scheme_type):
        self.term = term
        self.nvars = nvars
    ...
    def scheme_build(self):
        """
        building scheme, start from list of zeros where them numbers equal
        nvars. After that we move value in that axis which corresponding to
        term. [0,0]->[[1,0],[-1,0]] it means that term was [0] (d/dx) and mode
        (scheme_type) is 'central'.

        return: list
            numerical scheme list,
            example: [[1,0],[-1,0]], where [1,0] and [-1,0] pattern for
            grid mooving
    ...
    def sign_order(self, h=1/2):
        ...
        Parameters:
        ----------
        h: float, (default: h=1/2)
            step in numerical scheme.
            example: du/dx -> (u(x+h)-u(x))/h for 'f' scheme.
        ----------
        return: list
            values list: if scheme is [[1,0],[-1,0]] and h=1/2 ->
            result sign list is [[+1,-1]]
        """
...
class Second_order_scheme():
    ...
    ----------
    axes_scheme_type: string
        scheme type: combination of 'f' and 'b'
    ...
    def scheme_build(self):
        """
        Scheme building for Crank-Nicolson varint, it's identical to
    ...
    def sign_order(self, h=1/2):
        ...
        -----------
        return: list
            list with +/- values that corresponding to numerical scheme
...
class Finite_diffs():
    ...
    def scheme_choose(self, scheme_label, h=1/2):
        """
        ...
        ----------
        h: float, default: h=1/2
            scheme step, example: du/dx -> (u(x+h)-u(x))/h for 'f' scheme.
        ----------
        return: list
