...
class Learn:
    def __init__(self, solution, optimizer, device: str, mixed_precision: bool = True, **params):
        self.solution_cls = solution
        self.optimizer = optimizer
        self.device = device
        self.mixed_precision = mixed_precision
        self.second_order_interactions = params['second_order_interactions']
        self.sampling_N = params['sampling_N']
        self.lambda_update = params['lambda_update']
        self.tol = params['tol']
        if device == 'cuda':
            self.scaler = torch.cuda.amp.GradScaler(enabled=mixed_precision)

    def loss(self):
        loss, loss_normalized = self.solution_cls.evaluate(second_order_interactions=self.second_order_interactions,
                                                           sampling_N=self.sampling_N,
                                                           lambda_update=self.lambda_update,
                                                           tol=self.tol)
        return loss, loss_normalized

    def closure(self):
        self.optimizer.zero_grad()
        loss, loss_normalized = self.loss()
        loss.backward()
        self.optimizer.step()
        return loss, loss_normalized

    def closure_mixed_cuda(self):
        self.optimizer.zero_grad()
