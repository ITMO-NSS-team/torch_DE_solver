...
t = torch.from_numpy(np.linspace(0, 4*np.pi, 1000))
...
for _ in range(1):
    ...
    start = time.time()
    model_true = point_sort_shift_solver(grid, model, op, bconds, lambda_bound=1000, verbose=True, learning_rate=1e-5,
                                    eps=1e-7, tmin=1000, tmax=3e5,use_cache=False,cache_dir='../cache/',cache_verbose=True
