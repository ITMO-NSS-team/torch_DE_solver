...
def navier_stokes_2d_TG_vortex_experiment(grid_res):
    ...
    exact_u = exact_func_u(grid).reshape(-1, 1)
    exact_v = exact_func_v(grid).reshape(-1, 1)
    exact_p = exact_func_p(grid).reshape(-1, 1)
    ...
    exp_dict_list_u.append({
        ...
        'type': 'navier_stokes_2d_TG_vortex',
        'cache': True
    })
...
for grid_res in range(100, 1001, 100):
    for _ in range(nruns):
        list_u, list_v, list_p = navier_stokes_2d_TG_vortex_experiment(grid_res)
...
df_u.to_csv('examples/benchmarking_data/navier_stokes_2d_TG_vortex_experiment_100_1000_cache_u_func={}.csv'
            .format(str(True)))
df_v.to_csv('examples/benchmarking_data/navier_stokes_2d_TG_vortex_experiment_100_1000_cache_v_func={}.csv'
            .format(str(True)))
df_p.to_csv('examples/benchmarking_data/navier_stokes_2d_TG_vortex_experiment_100_1000_cache_p_func={}.csv'
            .format(str(True)))
