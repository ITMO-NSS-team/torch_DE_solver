...
# u(x_min, y, t) =  sin(pi * y) * (A1 * sin(pi * t) + A2 * sin(3pi * t) + A3 * sin(5pi * t))
boundaries.dirichlet({'x': x_min, 'y': [y_min, y_max], 't': [0, t_max]},
                     value=torch.sin(torch.pi * y) * (A1 * torch.sin(torch.pi * t) +
                                                      A2 * torch.sin(3 * torch.pi * t) +
                                                      A3 * torch.sin(5 * torch.pi * t)),
                     var=0)
