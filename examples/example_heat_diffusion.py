...
r = torch.from_numpy(np.array([0.7, 1.2, 1.7, 2.2, 2.7]))
t = torch.from_numpy(np.arange(0,1,1/dat.shape[1]))
...
r = torch.from_numpy(np.arange(0.7,2.7,2/100))
...
# u(0,x)=sin(pi*x)
bndval1 =torch.from_numpy( dat1[:,0])
...
model = point_sort_shift_solver(grid, model, heat, bconds, lambda_bound=1000, verbose=2,h=0.01, learning_rate=1e-5,
