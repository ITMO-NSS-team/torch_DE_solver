...
full_prepared_operator = operator_prepare(wave_eq, grid_dict, subset=['central'], true_grid=grid, h=0.01)
prepared_bconds = bnd_prepare(bconds,grid,grid_dict,h=0.01)
...
while (t<1e4):
    ...
    def closure():
        ...
        loss = torch.mean(op**2) + lambda_bound * torch.mean(residual**2)
        loss.backward()
        return loss
    ...
    print('t={}'.format(t))
    print(torch.mean(model(bnd3_left)-model(bnd3_right)))
    print(torch.mean(op**2))

        
    t+=1

solution_print(prepared_grid,model)