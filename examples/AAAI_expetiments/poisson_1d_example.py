...
def experiment_data_amount_poisson_1d_fourier(N,exp_name='poisson1d_fourier',save_plot=True):
    ...
    exp_dict_list.append({'grid_res': N, 'l2_loss': l2_loss, 'l2_norm': l2_norm_fourier(net, grid_error), 'type':'Poisson_Fourier_layers'})

    print('grid_res=', N)
    # print('c2_norm = ', c2_norm(net, grid_error))
    print('l2_norm = ', l2_norm_fourier(net, grid_error))
...
def experiment_data_amount_poisson_1d_NGD(grid_res,NGD_info_string=True,exp_name='poisson1d_NGD',save_plot=True):
    ...
    for iteration in range(100):
        ...
        f_nat_grad = np.linalg.lstsq(G, f_grads,rcond=-1)[0] #for some reason torch.linalg.lstsq does not work well
        f_nat_grad = torch.from_numpy(np.array(f_nat_grad)).to(torch.float64).to('cuda')

        #f_nat_grad=torch.linalg.lstsq(G, f_grads,rcond=-1)[0] #again this bit does not work well
...
if __name__ == '__main__':
    ...
    for N in grid_n:
        for _ in range(nruns):
            exp_dict_list.append(experiment_data_amount_poisson_1d_NGD(N,NGD_info_string=False))
