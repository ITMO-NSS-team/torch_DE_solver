...
def experiment_data_amount_kdv_adam_lbfgs_nncg(grid_res,exp_name='burgers1d_adam_lbfgs_nncg',save_plot=True):
    solver_device('cuda')
    exp_dict_list = []
    ...
    start = time.time()
    model.train(optim, 1000, save_model=False, callbacks=[cb_es], info_string_every=100)
    end = time.time()
    ...
    start = time.time()
    model.train(optim, 3000, save_model=False, callbacks=[cb_es], info_string_every=20)
    end = time.time()
    time_NNCG = end - start
...
if __name__ == '__main__':
    ...
    for grid_res in range(10, 101, 10):
        for _ in range(nruns):
            exp_dict_list.append(experiment_data_amount_kdv_adam_lbfgs_nncg(grid_res))
            exp_dict_list_flatten = [item for sublist in exp_dict_list for item in sublist]
            df = pd.DataFrame(exp_dict_list_flatten)
            df.to_csv('examples\\AAAI_expetiments\\results\\kdv_adam_lbfgs_nncg_{}.csv'.format(grid_res))
