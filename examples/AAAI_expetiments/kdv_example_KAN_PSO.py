...
def experiment_data_amount_kdv_PSO_KAN(grid_res,exp_name='kdv_PSO_KAN',save_plot=True):
    solver_device('cuda')
    exp_dict_list = []
...
if __name__ == '__main__':
    ...
    for grid_res in range(20, 101, 10):
        for _ in range(nruns):
            exp_dict_list.append(experiment_data_amount_kdv_PSO_KAN(grid_res))
            exp_dict_list_flatten = [item for sublist in exp_dict_list for item in sublist]
            df = pd.DataFrame(exp_dict_list_flatten)
            df.to_csv('examples\\AAAI_expetiments\\results\\kdv_PSO_KAN_{}.csv'.format(grid_res))
