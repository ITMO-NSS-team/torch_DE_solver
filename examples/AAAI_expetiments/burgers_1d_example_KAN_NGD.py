...
def experiment_data_amount_burgers_1d_NGD_KAN(grid_res,NGD_info_string=True,exp_name='burgers1d_NGD_KAN',loss_window=20,randomize_parameter=1e-6,save_plot=True):
...
if __name__ == '__main__':
    ...
    for grid_res in range(10, 61, 10):
        for _ in range(nruns):
            exp_dict_list.append(experiment_data_amount_burgers_1d_NGD_KAN(grid_res,NGD_info_string=True))
            exp_dict_list_flatten = [item for sublist in exp_dict_list for item in sublist]
            df = pd.DataFrame(exp_dict_list_flatten)
            df.to_csv('examples\\AAAI_expetiments\\results\\burgers_NGD_res_KAN_{}.csv'.format(grid_res))
