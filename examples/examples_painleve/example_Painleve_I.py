...
def p_I_exp(grid_res, nruns, CACHE):
    ...
    for _ in range(nruns):
        sln = np.genfromtxt(
            os.path.abspath(os.path.join(os.path.dirname(__file__), '../wolfram_sln/P_I_sln_' + str(grid_res) + '.csv')),
            delimiter=',')
        ...
        exp_dict_list.append(
            {'grid_res': grid_res, 'time': end - start, 'RMSE': error_rmse.detach().cpu().numpy(), 'type': 'PI',
             'cache': CACHE})
