...
data_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "../PINNacle_data/lid_driven_a4.npy"))
...
def navier_stokes_2d_lid_driven_flow_experiment(grid_res):
    ...
    # Left, right and bottom boundary: Γ2

    # u(x_min, y, t) = 0
    boundaries.dirichlet({'x': x_min, 'y': [y_min, y_max], 't': [0, t_max]}, value=0, var=0)
    # u(x_max, y, t) = 0
    ...
    # v(x, y_min, t) = 0
    boundaries.dirichlet({'x': [x_min, x_max], 'y': y_min, 't': [0, t_max]}, value=0, var=1)

    # p(x_min, y_min, t)
    boundaries.dirichlet({'x': x_min, 'y': y_min, 't': [0, t_max]}, value=0, var=2)
    ...
    img_dir = os.path.join(os.path.dirname(__file__), 'navier_stokes_2d_lid_driver_flow_img')
    ...
    exp_dict_list_u.append({
        ...
        'type': 'navier_stokes_2d_lid_driven_flow',
        'cache': True
    })
...
for grid_res in range(20, 201, 20):
    for _ in range(nruns):
        list_u, list_v, list_p = navier_stokes_2d_lid_driven_flow_experiment(grid_res)
...
df_u.to_csv('examples/benchmarking_data/navier_stokes_2d_lid_driven_flow_experiment_20_200_cache_u_func={}.csv'
            .format(str(True)))
df_v.to_csv('examples/benchmarking_data/navier_stokes_2d_lid_driven_flow_experiment_20_200_cache_v_func={}.csv'
            .format(str(True)))
df_p.to_csv('examples/benchmarking_data/navier_stokes_2d_lid_driven_flow_experiment_20_200_cache_p_func={}.csv'
            .format(str(True)))
