...
for grid_res in [10,20,30]:
    ...
    for _ in range(10):
        ...
        start = time.time()
        model = point_sort_shift_solver(grid, model, kdv, bconds, lambda_bound=100,verbose=1, learning_rate=1e-5,h=0.01,
                                        eps=1e-5, tmin=1000, tmax=1e5,use_cache=True,cache_verbose=True,
                                    batch_size=None, save_always=True,print_every=1000)
