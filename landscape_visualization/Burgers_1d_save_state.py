...
def experiment_state_burgers_1d_adam_lbfgs(grid_res, exp_name='burgers1d_adam_lbfgs_nncg', save_plot=True):
    ...
    start = time.time()
    # path_to_parent_folder = r"C:\Users\Рустам\Documents\GitHub\torch_DE_solver_local\landscape_visualization_origin" \
    #                         r"\trajectories\burgers\adam_state_test"
    path_to_parent_folder = r"C:\Users\anate\Documents\GitHub\loss_landscape\torch_DE_solver\landscape_visualization" \
                            r"\landscape_visualization\trajectories\burgers\adam_state_test"
    path_to_folder = path_to_parent_folder + r"\optimizer_Adam"
    os.makedirs(path_to_folder, exist_ok=True)

    cb_sm = save_model.SaveModel(path_to_folder, every_step=50)
    model.train(optim, 100, save_model=False, callbacks=[cb_es, cb_sm], info_string_every=50)
    end = time.time()
    ...
    os.makedirs(path_to_folder, exist_ok=True)
    cb_sm = save_model.SaveModel(path_to_folder, every_step=50)
    model.train(optim, 50, save_model=False, callbacks=[cb_es, cb_sm], info_string_every=10)
    end = time.time()
